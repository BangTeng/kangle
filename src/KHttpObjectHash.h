	...
	*/
	inline KHttpObject *get(KHttpRequest *rq) {
		...
		if (node==NULL) {
			...
			vary.val = ((KHttpObject *)match_url_node->data)->BuildVary(rq);
			if (vary.val == NULL) {
				lock.Unlock();
				return NULL;
			}
		...
		while (obj) {
			...
			if ((rq->ctx->internal == (TEST(obj->index.flags, FLAG_RQ_INTERNAL)>0)) &&
				obj->uk.url->match_accept_encoding(rq->raw_url.encoding)) {
				if (!TEST(rq->filter_flags, RF_NO_DISK_CACHE) || (obj->data != NULL && obj->data->type == MEMORY_OBJECT)) {
					//hit cache
					if (hit_obj == NULL || obj->uk.url->encoding > hit_obj->uk.url->encoding) {
						hit_obj = obj;
					}
				}
			}
...
private:
	...
	{
		KUrl *url = ((KHttpObject *)node->data)->uk.url;
		for (;;) {
			KHttpObject *obj = (KHttpObject *)node->data;
			if (cmp_vary_key(obj->uk.vary, vary) != 0) {
				do {
					obj->Dead();
					update_url_vary_key(&obj->uk, vary?vary->key:NULL);
					obj = obj->next;
				} while (obj);
			}
			node = rb_next(node);
			if (node==NULL || ((KHttpObject *)(node->data))->uk.url->cmp(url) != 0) {
				break;
			}
		}
	}
